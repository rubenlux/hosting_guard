...
# Codes treated as "generic fallback" — superseded by any more specific code
...
def _sync_deploy_events(conn) -> dict:
    ...
    Supersede: if a specific code and a generic code (build_failed,
    npm_install_failed, unknown_deploy_error) are both open for the same
    user+repo target, the generic incident is resolved with reason
    "superseded_by_specific_deploy_error".

    Resolve: when a success deploy_event follows, incidents are resolved with
    reason "deploy_success". Otherwise "no_recent_failure" after 7 days.
    """
    ...
    # ── Post-loop: load current open incidents for supersede + resolve ────────
    ...
    # Step B: resolve by absence — use "deploy_success" reason when applicable
    try:
        _success_targets = {
