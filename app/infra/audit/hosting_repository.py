...
import logging
from app.infra.db import get_connection, SQL_MINUTES_SINCE_CREATED, SQL_DAYS_REMAINING_14
...
class HostingRepository:
    ...
    def get_orchestrator_events(self, user_id: int, limit: int = 20, skip: int = 0) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT * FROM orchestrator_events WHERE user_id = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
            (user_id, limit, skip)
        )
        return [dict(row) for row in cursor.fetchall()]
    ...
    def get_last_event_by_type(self, container_name: str, event_type: str) -> Optional[Dict]:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM orchestrator_events WHERE container_name = %s AND event_type = %s ORDER BY created_at DESC LIMIT 1",
