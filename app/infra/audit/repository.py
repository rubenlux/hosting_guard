    ...
    def save_decision_event(self, tenant_id: str, decision: Dict, advisory: Dict) -> DecisionEvent:
        try:
            ...
            event = DecisionEvent(
                ...
                payload_min={"actions_count": len(decision.get("actions_evaluation", []))},
            )
        ...
        try:
            ...
            cursor.execute(
                ...
                """,
                (
                    event.event_id, event.timestamp.isoformat(), event.tenant_id, event.decision_id,
                    event.overall_status, event.confidence_level, int(event.requires_human_attention),
                    json.dumps(event.payload_min), event.version,
                ),
            )
            conn.commit()
            return event
