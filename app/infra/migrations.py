...
_INDEXES = [
    ...
    # ── Phase 4A: Auto-remediation executions — auditable, reversible, scoped ──
    """CREATE TABLE IF NOT EXISTS remediation_executions (
        remediation_id          BIGSERIAL PRIMARY KEY,
        hosting_id              BIGINT REFERENCES hostings(hosting_id) ON DELETE SET NULL,
        user_id                 BIGINT REFERENCES users(user_id) ON DELETE SET NULL,
        incident_id             BIGINT REFERENCES system_incidents(incident_id) ON DELETE SET NULL,
        security_event_id       BIGINT REFERENCES security_events(event_id) ON DELETE SET NULL,
        action_id               BIGINT REFERENCES action_recommendations(action_id) ON DELETE SET NULL,
        plan_id                 BIGINT REFERENCES execution_plans(plan_id) ON DELETE SET NULL,
        ...
        blocked_by_policy_reason TEXT,
        created_by              TEXT DEFAULT 'system',
        created_at              TIMESTAMPTZ DEFAULT NOW(),
        updated_at              TIMESTAMPTZ DEFAULT NOW()
    )""",
    "CREATE INDEX IF NOT EXISTS idx_remediations_hosting    ON remediation_executions(hosting_id, created_at DESC)",
    "CREATE INDEX IF NOT EXISTS idx_remediations_incident   ON remediation_executions(incident_id) WHERE incident_id IS NOT NULL",
    "CREATE INDEX IF NOT EXISTS idx_remediations_sec_event  ON remediation_executions(security_event_id) WHERE security_event_id IS NOT NULL",
    "CREATE INDEX IF NOT EXISTS idx_remediations_type       ON remediation_executions(remediation_type, created_at DESC)",
    "CREATE INDEX IF NOT EXISTS idx_remediations_target     ON remediation_executions(target_type, target_value)",
