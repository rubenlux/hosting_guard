...
Eliminé directorios temporales de backups de refactor: app_backup_refactor/ y scripts tmp_*.py en la raíz.
...
@property
    def orchestrator(self):
        if self._orchestrator is None:
            # Lazy load for background workers or tests to ensure it is NEVER None
...
SELECT 
    ...
    (SELECT COUNT(*) FROM bounce_calc) as total_sessions,
...
FROM pixel_events
WHERE site_id = %s AND created_at >= %s;
2. Query de Bounce Rate (Aislada y Segura)
