...
export default function SparklineChart({ data }) {
  if (!data || data.length < 2) {
    return <div className="h-[48px] bg-[#0a0a0c] border border-white/10 rounded-lg animate-pulse" />;
  }
  ...
  return (
    <div className="bg-[#121214] border border-white/10 rounded-lg p-4 shadow-sm">
      <svg
        viewBox={`0 0 ${W} ${H}`}
        className="w-full"
        style={{ height: 48 }}
